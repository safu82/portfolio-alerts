...
def main():
    ...
    # Get access token from Supabase (refreshed daily by GitHub Actions)
    print("🔑 Fetching access token from Supabase...")
    access_token = get_access_token_from_supabase()
    
    if not access_token:
        print("❌ No access token found in Supabase!")
        print("Make sure GitHub Actions ran successfully to generate the token.")
    ...
    while True:
        try:
            # Refresh token every 6 hours (in case GitHub Actions updated it)
            if time.time() - last_token_refresh > 21600:
                print("\n🔄 Refreshing access token from Supabase...")
                new_token = get_access_token_from_supabase()
