...
import re  # For regex pattern matching in symbol extraction
...
def get_recent_promoter_transactions() -> List[Dict]:
    ...
    Returns list of promoter transactions from last 15 days
    """
    # Use general URL (all stocks, not just Nifty 500) - matches working tracker
    ...
    try:
        ...
        if response.status_code == 200:
            # Parse with 15-day lookback window
            return parse_promoter_transactions(response.text, days_back=15)
        else:
...
def parse_promoter_transactions(html: str, days_back: int = 15) -> List[Dict]:
    """
    ...
    try:
        ...
        total_parsed = 0
        filtered_count = 0
        non_promoter_count = 0
        
        for row in rows:
            cols = row.find_all('td')
            if len(cols) < 7:  # Need at least 7 columns (matches working tracker)
                continue
            
            try:
                total_parsed += 1
                
                # COLUMN MAPPING (matches working telegram_tracker.py):
                # Column 0: Company Name (with link)
                # Column 1: Person/Entity Name
                # Column 2: Category (Promoter/Designated Person/etc.)
                # Column 3: Transaction Type (Acquisition/Disposal)
                # Column 4: Date
                # Column 5: Number of Shares
                # Column 6: Post-Holding %
                # Column 7: % Change (optional)
                
                # Extract company name and symbol from link
                ...
                # Extract symbol from URL (e.g., /equity/RELIANCE/...)
                symbol_match = re.search(r'/([A-Z0-9]+)/', company_url)
                symbol = symbol_match.group(1) if symbol_match else company_name
                
                # Person/Entity
                person_link = cols[1].find('a')
                person_entity = person_link.text.strip() if person_link else cols[1].text.strip()
                
                # Category (Promoter, Designated Person, Trust, etc.)
                ...
                # Post-Holding %
                post_holding = cols[6].text.strip()
                
                # % Change (if available)
                pct_change = cols[7].text.strip() if len(cols) > 7 else ''
                
                # Filter by category - only include promoters (matches working tracker)
                # Category contains: "Promoter", "Promoter Group", "Designated Person", "Trust", etc.
                is_promoter = 'promoter' in category.lower()
                
                if not is_promoter:
                    non_promoter_count += 1
                    if total_parsed <= 5:
                        print(f"  ⏭️  Skipping non-promoter: '{category}'")
                    continue
                ...
                # Symbol already extracted above via regex from company_url
                # Now append the transaction
                ...
                transactions.append({
                    ...
                    'symbol': symbol.upper(),
                    'person_entity': person_entity,  # Person/Entity making transaction
                    'category': category,  # Promoter/Designated Person/etc.
                    'transaction_type': transaction_type,  # Acquisition/Disposal
                    'shares': shares,  # Number of shares
                    'post_holding': post_holding,  # Post-holding %
                    'pct_change': pct_change  # % change
                })
            except Exception as e:
                continue
        ...
        print(f"  👥 Filtered out {non_promoter_count} non-promoter transactions (Designated Person, Other, etc.)")
        print(f"  ✅ Kept {len(transactions)} recent promoter transactions")
...
def scan_promoter_activity(supabase: Client) -> List[Dict]:
    """
    Scan for promoter activity (buying & selling) in ALL stocks
    Returns list of promoter activity signals
    """
    print("\n📊 Scanning for Promoter Activity...")
    
    # Get all recent promoter transactions (all stocks, not just Nifty 500)
    ...
    print(f"\n  🔍 DEBUG - Promoter transactions found:")
    for i, txn in enumerate(all_transactions[:10], 1):  # Show first 10
        print(f"    {i}. {txn['symbol']} - {txn['category']} - {txn['transaction_type']} ({txn['date']})")
    if len(all_transactions) > 10:
        print(f"    ... and {len(all_transactions) - 10} more")
    
    signals = []
    seen_tickers = set()  # Track which stocks we've already added
    
    for txn in all_transactions:
        # Validate ticker - skip if it's just numbers or too short
        symbol = txn['symbol'].upper()
        if not symbol or len(symbol) < 2 or symbol.isdigit():
        ...
        # Map to allowed signal_strength values (database uses 'strong' and 'regular')
        if is_buying:
            signal_strength = 'strong'  # Promoter acquisition
        elif is_selling:
            signal_strength = 'regular'  # Promoter disposal (marked in notes with 🔴)
        else:
            signal_strength = 'regular'  # Other types
        ...
        # Skip if no price data available (stock not in our database)
        if current_price is None:
            if len(signals) < 5:  # Only show first few to avoid spam
                print(f"  ⏭️  Skipping {ticker} - no price data in database")
            continue
        
        # Structure signal to match exact format of other signals
        signal = {
            'ticker': ticker,
            ...
            'price': current_price,
            # Put all promoter-specific data in details field (like other signals do)
            'promoter_name': txn['person_entity'],
            'transaction_type': txn['transaction_type'],
            'transaction_date': txn['date'],
            'shares': txn.get('shares', 0),
            'post_holding': txn.get('post_holding', ''),
            'is_disposal': is_selling,
            'category': txn['category']
        }
        
        signals.append(signal)
        seen_tickers.add(ticker)  # Mark this ticker as processed
    
    print(f"  ✅ Generated {len(signals)} promoter activity signals (buying & selling)")
    
    skipped_count = len(all_transactions) - len(signals)
    if skipped_count > 0:
        print(f"  ⏭️  Skipped {skipped_count} signals (no price data or invalid ticker)")
    
    return signals  # Return signals for database save
...
Quality: {confirmation['quality_score']}/5"""
    ...
    signal = {
        'signal_type': '200_ema_retest',  # Fixed: was '200_ema_bounce_confirmed'
...
def save_signals_to_supabase(supabase: Client, signals: List[Dict]) -> bool:
    ...
    try:
        ...
        # DEBUG: Show first promoter signal being inserted
        promoter_records = [r for r in records if r['signal_type'] == 'promoter_buying']
        if promoter_records:
            print(f"\n🔍 DEBUG - First promoter signal to insert:")
            for key, value in promoter_records[0].items():
...
def main():
    ...
    promoter_signals = scan_promoter_activity(supabase)  # Now scans ALL stocks, not just Nifty 500
